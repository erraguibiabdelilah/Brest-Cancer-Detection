...
class OpenAIReportGenerator:
    def __init__(self, api_key: str = None):
        ...
        self.model = "gpt-3.5-turbo"  # Rapide et économique
    ...
    def generate_interpretation(self, is_positive: bool, confidence: float) -> str:
        """Generate clinical interpretation using OpenAI"""
...
"""
        ...
        try:
            print(f"   📝 Appel OpenAI pour interprétation (confiance: {confidence*100:.1f}%)...")
            result = self._call_openai(prompt)
            print(f"   ✅ Interprétation générée ({len(result)} caractères)")
            return result
        except Exception as e:
            print(f"   ⚠️ Erreur OpenAI interprétation: {e}")
    ...
    def generate_recommendations(self, is_positive: bool, confidence: float) -> List[str]:
        """Generate medical recommendations using OpenAI"""
...
"""
        ...
        try:
            print(f"   📝 Appel OpenAI pour recommandations...")
            result = self._call_openai(prompt)
            ...
            print(f"   ⚠️ Erreur OpenAI recommandations: {e}")
    ...
    def generate_detailed_findings(self, is_positive: bool, confidence: float) -> str:
        """Generate detailed findings description using OpenAI"""
...
"""
        ...
        try:
            print(f"   📝 Appel OpenAI pour observations détaillées...")
            result = self._call_openai(prompt)
            print(f"   ✅ Observations générées ({len(result)} caractères)")
            return result
        except Exception as e:
            print(f"   ⚠️ Erreur OpenAI observations: {e}")
    ...
    def _get_fallback_findings(self, is_positive: bool) -> str:
        """Fallback detailed findings if OpenAI fails"""
        if is_positive:
            return (
